...
func TestPipeline(t *testing.T) {
	...
	freeFlowJobs := []job{
		job(func(in, out chan interface{}) {
			fmt.Println("Birinwi job kirildi jane de outka jazdyk")
			out <- 1
		...
		job(func(in, out chan interface{}) {
			fmt.Println("Ekinwi job kirildi ")
			for _ = range in {
	...
	hashSignJobs := []job{
		job(func(in, out chan interface{}) {
			for _, fibNum := range inputData {
				fmt.Println("Birinwi job kanday znachen : ")
				fmt.Println(fibNum)

				out <- fibNum
				fmt.Println("outka jazdy : ")
			}
		}),
		job(SingleHash),
		//job(MultiHash),
		//job(CombineResults),
		job(func(in, out chan interface{}) {
			fmt.Println("Ekinwi job  : ")
			dataRaw := <-in
			data, ok := dataRaw.(string)
			fmt.Println("Ekinwi job kanday znachen : " + data)
			if !ok {
